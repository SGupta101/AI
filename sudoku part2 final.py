...
def csp_backtracking_with_fl_and_cp(board, state):
    ...
    for val in get_sorted_values(state, var, len(state)):
        ...
        new_state[var] = val
        new_board[var] = val
        forward_board_and_state = forward_looking(new_board, new_state, [var])
        if forward_board_and_state is not None:
            forward_board = forward_board_and_state[0]
            forward_state = forward_board_and_state[1]
            constraint_board_and_state = constraint_propagation(forward_board, forward_state)
            if constraint_board_and_state is not None:
                constraint_board = constraint_board_and_state[0]
                constraint_state = constraint_board_and_state[1]
                # print("state:", state)
                ...
                # print("new_board:", new_board)
                # print("checked_board:", checked_board)
                result = csp_backtracking_with_fl_and_cp(constraint_board, constraint_state)
                if result is not None:
                    return result
    return None
