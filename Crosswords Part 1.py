...
def csp_backtracking(board, block_indexes):
    if board.count("#") == num_blocked_squares:
        ...
        # print_puzzle(board)
        return board
    values = get_possible_vals(board)
    for val in values:
...
def csp_backtracking_two(board, block_indexes):
    if board.count("#") == num_blocked_squares:
        # print("done")
        # print_puzzle(board)
        return board
    ...
    for val in values:
        ...
        if new_board_and_indexes is not None:
            ...
            if new_board_implied.count("#") <= num_blocked_squares and check_connected(new_board):
                # print("valid board")
                # print_puzzle(new_board)
                result = csp_backtracking_two(new_board_implied, new_indexes_implied)
...
def blocks_next_to(board, block_indexes):  # places blocks where no legal word can be made
    ...
    for block_index in block_indexes:
        index_row_and_column = find_pos(block_index)
        ...
        if valid_index(new_index):
            if row == index_row and board[new_index] == "#":
                ...
                # print_puzzle(board)
        new_index = block_index + my_width
        if valid_index(new_index):
            if find_pos(new_index)[0] == my_height - 1:
                ...
                # print("k", new_index)
                # print_puzzle(board)
        new_index = block_index + (my_width * 2)
        new_index_row_and_col = find_pos(new_index)
        column = new_index_row_and_col[1]
        row = new_index_row_and_col[0]
        if valid_index(new_index):
            if row == my_height - 1 or column == index_col and board[new_index] == "#":
                new_block_index = block_index + my_width
                board[new_block_index] = "#"
                ...
                # print_puzzle(board)
        new_index = block_index + (my_width * 3)
        new_index_row_and_col = find_pos(new_index)
        column = new_index_row_and_col[1]
        row = new_index_row_and_col[0]
        if valid_index(new_index):
            if column == index_col and board[new_index] == "#":
                ...
                new_block_index = block_index + (my_width * 2)
                board[new_block_index] = "#"
                added_block_indexes.add(new_block_index)
...
def area_fill_three(board, r, c, ch):
    ...
    if value == ch:
        board[index] = "*"
    area_fill_three(board, r + 1, c, ch)
    area_fill_three(board, r - 1, c, ch)
    area_fill_three(board, r, c + 1, ch)
    area_fill_three(board, r, c - 1, ch)
...
if len(strings_of_chars) == 0 and num_blocked_squares == my_height * my_width:
    ...
    if (
            my_height * my_width) % 2 == 1 and num_blocked_squares % 2 == 1:  # checks whether num_blocks is odd/even and places block at center based on that
    ...
    my_crossword_list_and_indexes = implied_blocks(my_crossword_list, my_block_indexes)
    my_crossword_list = my_crossword_list_and_indexes[0]
    my_block_indexes = my_crossword_list_and_indexes[1]
    final_crossword = csp_backtracking_two(my_crossword_list, my_block_indexes)
